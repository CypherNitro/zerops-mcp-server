#!/bin/sh
set -e
echo "=== Redeploy v12-oauth-noauth ==="
...
docker build --no-cache -t android-re-mcp:v12 /tmp/zerops-mcp-server-main
...
docker stop $(docker ps -q --filter ancestor=android-re-mcp:v12) 2>/dev/null || true
echo "Starting new container..."
docker run -d --network=host -e PORT=8080 -e EW_API_TOKEN=$EW_API_TOKEN android-re-mcp:v12
echo ""
echo "=== DONE! ==="
echo "Testing health..."
sleep 3
